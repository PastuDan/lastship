...
class App extends Component {
  ...
  upsLogin = () => {
    ipcRenderer.send(channels.LOGIN, {
      url: 'https://www.ups.com/lasso/login',
    })
    ipcRenderer.on(channels.LOGIN, (event, arg) => {
      ipcRenderer.removeAllListeners(channels.LOGIN)
      console.log('UPS LOGIN EVENT', event, arg)
      // this.setState({ appName, appVersion })
    })
  }

  render() {
    const { appName, appVersion, ups, fedex, usps } = this.state
    return (
      <div className="App">
        <header className="App-header">
          <svg className="App-logo" fill="currentColor" viewBox="0 0 49.641 49.64">
            <path
              d="M44.118,11.852c-0.008-0.014-0.022-0.021-0.03-0.034c-0.031-0.046-0.067-0.085-0.109-0.122
        ...
        {ups ? <div>Logged into UPS as {ups}</div> : <a onClick={this.upsLogin}>UPS Login</a>}
        <div>FedEx Login</div>
        <div>USPS Login</div>
      </div>
    )
